		...
		e.module("FileManagerApp").controller("FileManagerCtrl", ["$scope", "$rootScope", "$window", "$translate", "fileManagerConfig", "item", "fileNavigator", "apiMiddleware",
			function (e, n, i, a, t, o, s, l) {
				...
				e.config = t,
					...
					$.cookie('path_tmp') != null && (e.fileNavigator.currentPath = $.cookie('path_tmp').split(",")),
					e.uploadFileList = [],
					...
					e.prepareNewGraph = function () {
						var item = new o(null, e.fileNavigator.currentPath);
						e.temps = [item];
						return item;
					},
					e.smartClick = function (r) {
						...
						if (r.isXml()) {
							var path = e.apiMiddleware.getFilePath(e.singleSelection());
							$.cookie("file_tmp", path);
							location = e.config.editorUrl;
						}
						if (r.isEditable()) {
					...
					e.openImagePreview = function () {
						...
						e.apiMiddleware.apiHandler.inprocess = true;
						e.modal('imagepreview', null, true)
							.find('#imagepreview-target')
							.attr('src', "/File/Preview/?path=" + r.model.fullPath() + "&isXml=true")
							.unbind('load error')
							.on('load error', function () {
								e.apiMiddleware.apiHandler.inprocess = false;
								e.$apply();
							});

						// if(r.model.pic==""){
						// 	location.href = "/File/XmlThumb/?path=" + r.model.fullPath();
					...
					e.createGraph = function () {
						var item = e.singleSelection();
						var name = item.tempModel.name;
						if (name && !name.toLowerCase().endWith('.xml')) {
							name = name + '.xml';
						}
						if (!name || e.fileNavigator.fileNameExists(name)) {
						...
						uploader.bind('FileUploaded', function (up, file) {
							...
							e.fileNavigator.refresh();
							e.modal('newgraph', true);
							getMemory();
							var paths = $.cookie("path_tmp").split(",");
							var path = "/";
		...
		e.module("FileManagerApp").factory("item", ["fileManagerConfig", "chmod",
			function (r, n) {
				...
				return i.prototype.update = function () {
					...
					i.prototype.isXml = function () {
						return r.isXmlFilePattern.test(this.model.name)
					},
					...
					i.prototype.encodePath = function(){
						return encodeURIComponent(this.model.fullPath());
					},
					i
			}
		])
		...
		e.module("FileManagerApp").provider("fileManagerConfig",
			function () {
				var r = {
					...
					isEditableFilePattern: /\.(txt|diff?|patch|svg|asc|cnf|cfg|conf|html?|.html|cfm|cgi|aspx?|ini|pl|py|css|cs|js|jsp|log|htaccess|htpasswd|gitignore|gitattributes|env|json|atom|eml|rss|markdown|sql|xslt?|sh|rb|as|bat|cmd|cob|for|ftn|frm|frx|inc|lisp|scm|coffee|php[3-6]?|java|c|cbl|go|h|scala|vb|tmpl|lock|go|yml|yaml|tsv|lst)$/i,
			...
			function (e) {
				e.useSanitizeValueStrategy(null),
					e.translations("zh_cn", {
						...
						date: "日期",
						selection: "选择",
						no_files_in_folder: "文件夹中没有图表，可以点击右上角加号新建图表",
		...
		function (e) {
			e.put("src/templates/current-folder-breadcrumb.html", '<ol class="breadcrumb">\r\n    <li>\r\n        <a href="" class="wave_hide" ng-click="fileNavigator.goTo(-1)">\r\n            {{"filemanager" | translate}}\r\n        </a>\r\n    </li>\r\n    <li ng-repeat="(key, dir) in fileNavigator.currentPath track by key" ng-class="{\'active\':$last}" class="animated fast fadeIn">\r\n        <a href="" ng-show="!$last" ng-click="fileNavigator.goTo(key)" class="notWave">\r\n            {{dir | strLimit : 8}}\r\n        </a>\r\n        <span ng-show="$last">\r\n            {{dir | strLimit : 12}}\r\n        </span>\r\n    </li>\r\n</ol>'),
				e.put("src/templates/item-context-menu.html",
					...
					+ '<li ng-show="config.allowedActions.shareFile && singleSelection() && !singleSelection().isFolder()">\r\n            <a href="" tabindex="-1" ng-click="modal(\'share\')">\r\n             <i class="glyphicon glyphicon-share"></i> 分享\r\n            </a>\r\n        </li>'
					...
					+ '<li ng-show="config.allowedActions.preview && singleSelection().isAudio() && singleSelection()">\r\n            <a href="" tabindex="-1" ng-click="openAudioPreview()">\r\n                <i class="glyphicon glyphicon-music"></i> 预览音频\r\n            </a>\r\n        </li> '
					+ '<li ng-show="config.allowedActions.preview && singleSelection().isDoc() && singleSelection()">\r\n            <a href="" tabindex="-1" ng-click="openDocPreview()">\r\n                <i class="fa fa-file-word-o"></i> 预览文档\r\n            </a>\r\n        </li>  '
					+ '<li ng-show="config.allowedActions.preview && singleSelection().isGoogleDoc() && singleSelection()">\r\n            <a href="" tabindex="-1" ng-click="openGoogleDocPreview()">\r\n                <i class="fa fa-file-word-o"></i> 预览文档\r\n            </a>\r\n        </li> '
				...
				e.put("src/templates/modals.html",
					...
					+ '<div class="modal animated fadeIn" id="newgraph">\r\n  <div class="modal-dialog">\r\n    <div class="modal-content">\r\n        <form ng-submit="createGraph()">\r\n            <div class="modal-header">\r\n              <button type="button" class="close" data-dismiss="modal">\r\n                  <span aria-hidden="true">&times;</span>\r\n                  <span class="sr-only">{{"close" | translate}}</span>\r\n              </button>\r\n              <h4 class="modal-title">{{\'new_graph\' | translate}}</h4>\r\n            </div>\r\n            <div class="modal-body">\r\n              <label class="radio">{{\'graph_name\' | translate}}</label>\r\n              <input class="form-control" ng-model="singleSelection().tempModel.name" autofocus="autofocus">\r\n              <div ng-include data-src="\'error-bar\'" class="clearfix"></div>\r\n            </div>\r\n            <div class="modal-footer">\r\n              <button type="button" class="btn btn-default" data-dismiss="modal" ng-disabled="apiMiddleware.apiHandler.inprocess">{{"cancel" | translate}}</button>\r\n              <button type="submit" class="btn btn-primary" ng-disabled="apiMiddleware.apiHandler.inprocess">{{\'create\' | translate}}</button>\r\n            </div>\r\n        </form>\r\n    </div>\r\n  </div>\r\n</div>\r\n\r\n'
				...
				e.put("src/templates/navbar.html", '<nav class="navbar navbar-default lefts">'
					...
					+ '<button class="btn btn-primary btn-flat" ng-click="fileNavigator.upDir()"><i class="glyphicon glyphicon-chevron-left"></i></button>'
					...
					+ '</button>'
					+ '<button onclick="openUpload()" class="btn btn-flat btn-sm" id="up_button" title="{{\'upload\' | translate}}" ng-show="!config.allowedActions.allowRemoteDownload"><i class="glyphicon glyphicon-cloud-upload"></i>'
					+ '</button>'
					+ '<button class="btn btn-flat btn-sm" ng-click="modal(\'newgraph\') && prepareNewGraph()" title="{{\'new_graph\' | translate}}">'
