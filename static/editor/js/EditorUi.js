...
EditorUi.prototype.saveAndExit = function(forceDialog)
{
	this.exit = true;
	this.saveFile(forceDialog);
}
...
EditorUi.prototype.saveFile = function(forceDialog)
{
	...
	else
	{
		...
		{
			if(!name.toLowerCase().endWith('.xml'))
			{
				name = name + '.xml';
			}
			this.save(name);
			...
			if(this.editor.filename == name)
			{
				...
				$.ajax({
					...
					success: function(response){ 
						...
						{
							that.editor.setStatus(mxUtils.htmlEntities(mxResources.get('errorSavingFile') + ' ' +response.result.error));
							that.exit = false;
						}
					} ,
					error:function(e)
...
EditorUi.prototype.saveLocalFile = function(filename, data, mimeType, base64Encoded)
{
 ...
 *
 */
EditorUi.prototype.exportImage = function(filename,scale, background, border, format)
{
	...
	try
	{
		this.exportToCanvas(mxUtils.bind(this, function(canvas)
		{
			try
			{
				this.saveCanvas(filename,canvas, format);
			}
			catch (e)
			{
			}
		}), null, null, background, null, null, true, scale || 1, true,
			false, null, null, border, true);
	}
	catch (e)
	{
		this.handleError(e);
 ...
 * 
 */
EditorUi.prototype.saveCanvas = function(filename,canvas, format)
{
	var data = this.createImageDataUri(canvas, format);
	this.saveLocalFile(filename, data.substring(data.lastIndexOf(',') + 1), 'image/' + format, true);
};

/**
 * 
 */
EditorUi.prototype.createImageDataUri = function(canvas, format)
{
