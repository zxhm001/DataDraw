...
var AboutDialog = function(editorUi)
{
	...
	mxUtils.write(h3, mxResources.get('about') + ' 数字绘');
	div.appendChild(h3);
	...
	mxUtils.write(div, ' And ');
	var link2 = document.createElement('a');
	link2.setAttribute('href', 'https://cloudreve.org/');
	link2.setAttribute('target', '_blank');
	mxUtils.write(link2, 'Cloudreve');
...
//提示购买VIP的窗口
var VipDialog = function(editorUi){
	var div = document.createElement('div');
	div.setAttribute('align', 'center');
	...
	mxUtils.write(label, '更多图形是终身会员专享权益，邀请5个好友注册或付款99元即可成为会员');
	...
	var applyBtn = mxUtils.button('成为会员', function()
	{
...
ExportDialog.exportFile = function(editorUi, name, format, bg, s, b)
{
	...
	if (format == 'xml')
	{
    	editorUi.saveLocalFile(name, mxUtils.getXml(editorUi.editor.getGraphXml()), 'text/xml');
	}
    else if (format == 'svg')
	{
		editorUi.saveLocalFile(name, mxUtils.getXml(graph.getSvg(bg, s, b)), 'image/svg+xml');
	}
    else
    {
    	editorUi.exportImage(name,s,bg,b,format);
	}
};
...
var MoreShapesDialog = function(editorUi, expanded, entries) 
{
	...
	if (expanded)
	{
		var hd = document.createElement('div');
		hd.className = 'geDialogTitle';
		mxUtils.write(hd, mxResources.get('shapes') + '(会员)');
		hd.style.position = 'absolute';
		hd.style.top = '0px';
		hd.style.left = '0px';
		hd.style.lineHeight = '40px';
		...
		var applyBtn = mxUtils.button(mxResources.get('apply'), function()
		{
			if(!window.isVip)
			{
				editorUi.showDialog(new VipDialog(editorUi).container, 360,120, true, true);
				return;
			}
	    	editorUi.hideDialog();
	    	var libs = [];
